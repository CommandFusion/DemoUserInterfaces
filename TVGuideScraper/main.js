...
// Guide Setup
var minuteWidth = 2, borderWidth = 4, guideList = "l_guide";

var guideURL = "http://www.programme-tv.net/programme/canalsat-5/en-ce-moment.html";
...
function updateGuideList(startDate) {
	...
	for (var i = 0; i < guideData.channels.length; i++) {
        ...
        newListItem["s1"] = guideData.channels[i].name;
		...
		for (var j = 0; j < guideData.channels[i].shows.length; j++) {
			...
			newListItem["s" + (j + 2)] = guideData.channels[i].shows[j].title;
		}
	...
	for (var i = 0; i < guideData.channels.length; i++) {
		...
		var showSizes = [], nextX = 200 + borderWidth;
        // Resize the channel title entry
        showSizes.push({join: guideList+":"+i+":d1", w: 200, x: 0, opacity: 1});
        // Loop through all the shows in each channel
		for (var j = 0; j < guideData.channels[i].shows.length; j++) {
			...
			showSizes.push({join: guideList+":"+i+":d" + (j + 2), w: showWidth, x: nextX, opacity: 1});
		...
		for (var j = guideData.channels[i].shows.length; j <= 24; j++) {
			...
			showSizes.push({join: guideList+":"+i+":d" + (j + 2), opacity: 0});
		}
...
function scrapeGuide(url) {
    ...
    CF.request(url, function (status, headers, body) {
        ...
        if (status == 200) {
            ...
            CF.log("Scraper request failed with status " + status);
        }
    });
}

var timeRegex = /^(\d+) mn$/, hourRegex = /^(\d+)h$/, hourMinRegex = /^(\d+)h(\d+)$/;
function getMins(timeString) {
    var matches;
    if (matches = timeRegex.exec(timeString)) {
        return matches[1];
    } else if (matches = hourRegex.exec(timeString)) {
        return parseInt(matches[1], 10) * 60;
    } else if (matches = hourMinRegex.exec(timeString)) {
        return (matches[1] * 60) + parseInt(matches[2], 10);
    }
}

var decoderDiv = document.createElement('div');
function decodeEntities(encoded) {
    decoderDiv.innerHTML = encoded;
    return decoderDiv.firstChild.nodeValue;
}
